...
async update(id: string, data: Prisma.OrderUpdateInput): Promise<Order> {
    ...
    const updatedOrder: Order = {
        ...
        description: data.description !== undefined ? String(data.description) : currentOrder.description,
        imageUrl: data.imageUrl !== undefined ? String(data.imageUrl) : currentOrder.imageUrl,
        status: data.status !== undefined ? String(data.status) as any : currentOrder.status,
        processedAt: data.processedAt !== undefined
            ? (data.processedAt ? new Date(data.processedAt as any) : null)
